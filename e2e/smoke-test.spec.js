...
test.describe('OneFoodDialer Smoke Test', () => {
  ...
  test('Complete user flow: Login → Dashboard → CRM → Orders → Logout', async ({ page }) => {
    ...
    await test.step('CSV export functionality works', async () => {
      ...
      // Verify download
      expect(download.suggestedFilename()).toMatch(/customer_management_\d{4}-\d{2}-\d{2}_\d{2}-\d{2}-\d{2}\.csv/);
    });
    ...
    await test.step('Real-time dashboard updates work', async () => {
      ...
      const initialTime = await page.locator('text=Last Updated').locator('..').locator('div').nth(1).textContent();
      ...
      const currentTime = await page.locator('text=Last Updated').locator('..').locator('div').nth(1).textContent();
  ...
  test('Performance and loading states', async ({ page }) => {
    ...
    await test.step('Loading states are shown', async () => {
      ...
      const loadingIndicator = page.locator('[data-testid="loading-skeleton"], .animate-spin, text=Loading');
