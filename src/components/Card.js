...
export default function Card({
  ...
  if (loading) {
    return (
      <div
        className={`bg-white rounded-xl shadow-sm border border-gray-200 p-6 animate-pulse ${className}`}
      >
...
export function CustomCard({ title, color = 'blue', loading = false, onClick, children }) {
  return (
    <Card title={title} color={color} loading={loading} onClick={onClick}>
      {children}
    </Card>
  );
}
